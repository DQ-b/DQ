...
| 模块 | 能力 | 入口 |
| --- | --- | --- |
| `fuzzer` | **模糊测试**：HTTP 参数/路径/头部 fuzz + 通用字节级变异 | `sectoolkit fuzz` |
| `tools` | **调用现有安全工具**：封装 nmap / sqlmap，解析结果为统一报告 | `sectoolkit scan` |
| `pentest` | **自动化渗透测试**：侦察→端口扫描→Web fuzz→SQLi 验证 流水线 | `sectoolkit pentest` |
...
流程：`nmap 端口扫描 → 推导 Web 端口 → 目录探测 → 参数 fuzz → sqlmap 验证`。
任一外部工具缺失会自动跳过并在报告中记录，不中断流程。可用 `--no-portscan` /
`--no-dirbust` / `--no-fuzz` / `--no-sqlmap` 裁剪步骤。
...
├── fuzzer/
│   ├── engine.py     # HTTP fuzz 引擎（并发 + 基线比对）
│   ├── mutator.py    # 通用字节级变异器
